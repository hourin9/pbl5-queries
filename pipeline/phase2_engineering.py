        ...
        async with joern_lock:
            try:
                ...
                if not os.path.exists(tmp_json):
                    logger.error(f"Joern không tạo được file kết quả tại: {tmp_json}. Response: {res}")
                    return None
                ...
                state_manager.update_phase(repo_url, 2, 'FAILED', f"Joern error: {str(e)}")
                return None
...
def process_commit_history_worker(repo_path):
    ...
    try:
         for commit in Repository(repo_path, only_no_merge=True, only_modifications_with_file_types=exts).traverse_commits():
...
async def run_phase2_engineering(output_dir="method_evolutions", joern_endpoint="localhost:9000"):
    ...
    for rp in repos:
        ...
        async def process_one(r_url, r_name, l_path, r_out):
            ...
            if not static_metrics_list:
                state_manager.update_phase(r_url, 2, 'FAILED', "Joern parse thất bại")
                return
            ...
            result = await loop.run_in_executor(executor, process_commit_history_worker, l_path)
        ...
        tasks.append(asyncio.create_task(process_one(repo_url, repo_name, local_path, repo_output_dir)))
