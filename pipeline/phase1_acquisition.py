...
if not GITHUB_TOKENS:
    logger.warning("Không tìm thấy GITHUB_TOKEN trong .env. Quá trình fetch API có thể bị giới hạn.")
    GITHUB_TOKENS = [""]
...
class GitHubAcquisition:
    ...
    async def fetch_repos_page(self, session, query, page=1):
        ...
        params = {"q": query, "per_page": 100, "page": page, "sort": "stars", "order": "desc"}
    ...
    async def crawl_repositories(self, max_repos=100):
        ...
        forbidden = ['algorithm', 'leetcode', 'interview', 'exercise', 'tutorial', 
                     'course', 'solution', 'practice', 'competitive', 'demo', 
                     'template', 'syllabus', 'handbook', 'learning', 'example']
        ...
        async with aiohttp.ClientSession() as session:
            for query in queries:
                ...
                page = 1
                query_count = 0
                while len(collected_repos) < max_repos and query_count < REPOS_PER_QUERY:
                    try:
                        ...
                        for item in items:
                            ...
                            if len(collected_repos) >= max_repos:
                                break
                        page += 1
                        await asyncio.sleep(2)  # Nghỉ giữa mỗi query để tránh chặn rate limit
...
async def clone_repository(repo_url, repo_name, output_dir):
    try:
        ...
        cmd = f"git clone --depth 100 {repo_url} {local_path}"
...
async def run_phase1_acquisition(max_repos=100):
    ...
    for repo in collected:
         tasks.append(asyncio.create_task(sem_clone(repo["clone_url"], repo["name"], acq.output_dir)))
